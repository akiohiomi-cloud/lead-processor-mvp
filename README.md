...
### Що бачите в DRY_RUN-виводі

- **AI заглушений.** Без ключа Anthropic модель не викликається — класифікатор повертає фіксований `score=50`, `class=warm`, `summary=[DRY_RUN] ...`. Це не баг і не заглушка-помилка, це навмисна поведінка для безключового запуску. **Реальну класифікацію** (динамічний score, AI-summary українською, hot/warm/cold за змістом заявки) видно у Loom-демо та в real-mode.
- **HTML-теги у Telegram-повідомленні.** У логах сервера видно сирий текст із тегами (`<b>WARM</b>`, `<code>+380...</code>`) — це повідомлення таким і піде в Telegram. У реальному Telegram (з `parse_mode=HTML`) теги перетворюються на жирний текст і моноширинні блоки.
